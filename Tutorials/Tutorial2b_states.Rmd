...
# remove AK
cor(statedata[-2,"life.exp"], statedata[-2,"inc"])
```
...
abline(model1$coefficients, 
       lty=2, lwd=2.5, col=4)
obs.remove=sample(1:nrow(statedata), 1)
points(statedata$inc[obs.remove],
       statedata$life.exp[obs.remove],
       cex=3,col=2)
model2=lm(life.exp~inc, data=statedata[-obs.remove,])
model2
